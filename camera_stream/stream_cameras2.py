...
class CameraStatusBroadcaster:
    ...
    def get_status_message(self):
        ...
        for stream in self.streams:
            ...
            is_connected = os.path.exists(device) #Checking if the cam is really plugged in or not
...
class MultiCamRTSPServer:
    ...
    def register_camera(self, device_path: str, profiles: Dict[str, Any], default_profile: str):
        '''
        Registers a new camera device with the RTSP server.
        
        :param self: Description
        :param device_path: Description
        :type device_path: str
        :param profiles: Description
        :type profiles: Dict[str, Any]
        :param default_profile: Description
        :type default_profile: str
        '''
        mounts = self.server.get_mount_points()
        cam_name = device_path.split("/")[-1]
        new_streams = []

        for profile_name, profile in profiles.items():
            codec = str(profile.get("codec", "H264")).upper()
            route = _profile_route(f"/{cam_name}", profile_name, codec, default_profile)

            # Avoid double-adding if the device was already registered
            if mounts.match(route)[0] != 0:
                continue

            pipeline_str = build_pipeline_from_profile(device_path, profile)
            factory = GstRtspServer.RTSPMediaFactory()
            factory.set_protocols(GstRtsp.RTSPLowerTrans.UDP)
            factory.set_launch(pipeline_str)
            factory.set_shared(False)
            ...
            new_streams.append(new_stream_info)
            print(f"[New_camera] Registered: {route}")
        if new_streams:
            self.broadcaster.streams.extend(new_streams)
...
def detect_camera() -> List[str]:
    '''Uses pyudev to detect connected cameras'''

    context = pyudev.Context()
    cams = []

    #only taking video capturing devices
    # all devices are not capable of capturing video 
    for device in context.list_devices(subsystem='video4linux'):
        capability = device.get('ID_V4L_CAPABILITIES', '')
        if ':capture:' not in capability:
            continue
        
        #one camera can have multiple symlink
        symlinks = device.device_links
...
def camera_hotplug_monitor(rtsp_server, profiles, default_profile):
    context = pyudev.Context()
    ...
    def monitor_callback(device):
        if device.action == 'add':
                if ':capture:' in device.get('ID_V4L_CAPABILITIES', ''):
                    symlinks = device.device_links
                    for link in symlinks:
                        if "by-id" in link:
                            print(f"\nNew camera detected: {link}\n")
                            GLib.idle_add(rtsp_server.register_camera, link, profiles, default_profile)
                            break
...
if __name__ == '__main__':
    ...
    camera_config = {f"/{cam.split('/')[-1]}": cam for cam in cams}
    ...
    camera_observer = camera_hotplug_monitor(server, STREAM_PROFILES, DEFAULT_PROFILE)  
